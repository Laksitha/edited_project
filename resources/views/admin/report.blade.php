                ...
                <div class="col-lg-12 grid-margin stretch-card">
                        <div class="card">
                                <div class="card-body">
                                        ...
                                        <button class="btn btn-danger" id="employee-btn">Generate pdf</button>
                                         <p class="card-description"></p>
                                        <div class="table-responsive">
                                        <table class="table table-bordered">
                                                <thead>
...
<script>
    $(document).ready(function () {
        ...
        $('.month_select_box').change(function(){
            location.href = "{{ url('/admin/report/') }}?month="+$(this).val()+"&year="+$('.year_select_box').val();
        });

        $('.year_select_box').change(function(){
            location.href = "{{ url('/admin/report/') }}?month="+$('.month_select_box').val()+"&year="+$(this).val();
        });

        $('#employee-btn').click(function(){
            if($('#employee').val() != ''){
                window.open("{{ url('/admin/report/salarySlip/') }}/"+$('#employee').val()+'/'+$('.month_select_box').val()+'/'+$('.year_select_box').val(), '_blank');
            }
        $('#epf-btn').click(function(){
            if($('#employee').val() != ''){
                window.open("{{ url('/admin/report/epfReport/') }}/"+$('.month_select_box').val()+'/'+$('.year_select_box').val(), '_blank');
            }
        });
